...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) { try { await contextService.requirePermission("read", "alerts"); const alert = await operationalAlertManagementService.getAlertDetail(Number((await params).id)); if (!alert) return NextResponse.json({ message: "Alerta no encontrada." }, { status: 404 }); return NextResponse.json(alert); } catch (error) { return NextResponse.json({ message: error instanceof Error ? error.message : "No autorizado." }, { status: 403 }); } }
