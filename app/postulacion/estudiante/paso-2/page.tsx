...
export default function PasoDosEstudiantePage() {
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  
  // Estado para previsualización de la Carta de la Universidad (PDF o Imagen)
  const [cartaFile, setCartaFile] = useState<{ name: string; url: string; type: string } | null>(null);

  const handleSiguiente = (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    // Navegar al paso 3 (Pago/Finalización para estudiantes)
    setTimeout(() => {
        router.replace("/postulacion/estudiante/paso-3");
    }, 1000);
  };

  const handleAtras = () => {
    router.push("/postulacion/estudiante/paso-1");
  };

  // Función para capturar el archivo y generar previsualización
  const handleCartaChange = (e: ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file) {
      const objectUrl = URL.createObjectURL(file);
      setCartaFile({ name: file.name, url: objectUrl, type: file.type });
    }
  };
        ...
        <div className="max-w-5xl mx-auto w-full px-4 sm:px-6">
          ...
          <div className="pb-10">
              ...
              <p className="text-lg text-white/90 font-medium">
                  Registre los datos de su centro de estudios y adjunte su constancia.
              </p>
          </div>
          ...
          <div className="space-y-8">
            ...
            <section className="bg-white rounded-3xl border border-gray-200 overflow-hidden shadow-sm">
              ...
              <div className="px-8 py-5 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-lg font-bold text-[#2F3136] tracking-tight">Estudios Universitarios</h2>
                    <p className="text-xs text-gray-500 font-medium">Complete la información de su universidad o instituto.</p>
                  </div>
                </div>
              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                  ...
                  <div className="md:col-span-8">
                    <label className="text-xs font-bold text-slate-700 mb-1.5 block uppercase tracking-wide">
                      Universidad / Instituto <span className="text-[#8A1C2C]">*</span>
                    </label>
                    <select required className="w-full h-11 px-3 rounded-xl border border-gray-300 focus:border-[#C5A059] focus:ring-2 focus:ring-[#C5A059]/20 focus:outline-none font-medium text-sm text-slate-600 transition-all">
                  ...
                  <div className="md:col-span-4">
                    <label className="text-xs font-bold text-slate-700 mb-1.5 block uppercase tracking-wide">
                      Año de ingreso <span className="text-[#8A1C2C]">*</span>
                    </label>
                    <input
                      type="number"
                      required
                  ...
                  <div className="md:col-span-12">
                    <label className="text-xs font-bold text-slate-700 mb-1.5 block uppercase tracking-wide">
                      Especialidad <span className="text-[#8A1C2C]">*</span>
                    </label>
                    <input
                      type="text"
                      required
                      placeholder="Ej. Ingeniería de Minas, Geología, Metalurgia..."
                      className="w-full h-11 px-3 rounded-xl border border-gray-300 focus:border-[#C5A059] focus:ring-2 focus:ring-[#C5A059]/20 focus:outline-none font-medium text-sm text-slate-700 transition-all"
                    />
                  </div>
            ...
            {/* =========================================
                SECCIÓN 2: CARGA DE DOCUMENTOS Y TÉRMINOS
            ========================================= */}
                ...
                <div className="bg-[#FFFDF8] border border-[#E8D09E] rounded-xl p-5 mb-8 flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-[#7f561e] leading-relaxed">
                      La afiliación estudiantil es <strong>gratuita durante su periodo de pregrado</strong>. Para renovar su membresía anual, deberá presentar una constancia emitida por su centro de estudios, caso contrario perderá su membresía.
                    </p>
                  </div>
                </div>
                ...
                <div className="mb-8 pl-1">
                  <label className="flex items-start gap-3 cursor-pointer group">
                    ...
                    <span className="text-sm text-[#2F3136] font-medium transition-colors">
                      He leído y acepto las condiciones de la afiliación estudiantil gratuita.
                    </span>
                  </label>
                </div>

                {/* DROPZONE: CARTA DE LA UNIVERSIDAD */}
                <div>
                  <label className="text-xs font-bold text-slate-700 mb-3 block uppercase tracking-wide">
                    Adjuntar carta de la universidad donde realiza sus estudios <span className="text-[#8A1C2C]">* (Max: 1MB)</span>
                  </label>
                  ...
                  <label className="group cursor-pointer relative overflow-hidden h-[260px] w-full border-2 border-dashed border-gray-300 hover:border-[#C5A059] rounded-2xl p-6 transition-all duration-300 hover:bg-gray-50 flex flex-col items-center justify-center text-center">
                    <input type="file" accept=".pdf,.jpg,.jpeg,.png" required onChange={handleCartaChange} className="hidden" />
                    
                    {cartaFile ? (
                      cartaFile.type.startsWith('image/') ? (
                        // SI ES IMAGEN: MOSTRAR PREVISUALIZACIÓN VISUAL
                        <div className="w-full h-full absolute inset-0 bg-black/5 flex items-center justify-center">
                          <img src={cartaFile.url} alt="Carta Preview" className="w-full h-full object-cover opacity-90 group-hover:opacity-60 transition-opacity" />
                      ...
                      ) : cartaFile.type === 'application/pdf' ? (
                        ...
                        <div className="w-full h-full absolute inset-0 bg-white flex flex-col items-center justify-center overflow-hidden">
                          <iframe 
                            src={`${cartaFile.url}#toolbar=0&navpanes=0&scrollbar=0`} 
                            className="w-full h-full pointer-events-none" 
                            title="Carta PDF Preview" 
                          />
                        ...
                        <div className="flex flex-col items-center justify-center h-full w-full relative z-10">
                          ...
                          <p className="text-xs text-gray-500 mb-4 truncate w-full max-w-[200px]" title={cartaFile.name}>{cartaFile.name}</p>
                      ...
                      <>
                        ...
                        <h3 className="text-sm font-bold text-[#2F3136] mb-1">Adjuntar Constancia o Carta</h3>
                        <p className="text-xs text-gray-500 mb-4 max-w-[200px]">Archivos PDF, JPG o PNG. Peso máximo: 1MB.</p>
                        <div className="inline-flex items-center gap-2 text-[#C5A059] font-bold text-xs uppercase tracking-wider">
