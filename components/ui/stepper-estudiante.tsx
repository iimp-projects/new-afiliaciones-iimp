...
// ARREGLO EXCLUSIVO PARA ESTUDIANTES (3 PASOS)
const PASOS = [
  {
    id: 1,
...
export default function StepperEstudiante() {
  const pathname = usePathname();

  // Detecta el paso actual leyendo la URL
  const pasoActual = PASOS.find((p) => pathname.includes(p.path))?.id || 1;

  // Cálculo de la barra de progreso para 3 pasos (0%, 50%, 100%)
  ...
  return (
    <div className="w-full">
      ...
      <div className="hidden md:block relative">
        ...
        {/* CONTENEDOR DE 3 COLUMNAS (grid-cols-3) */}
        <div className="grid grid-cols-3 gap-4 lg:gap-6 relative">
          {PASOS.map((paso) => {
            const Icon = paso.icon;
            const active = paso.id === pasoActual;
            const completed = paso.id < pasoActual;
            ...
            return (
              ...
              >
                ...
                {/* Efecto de pulso luminoso para el actual */}
                {active && (
                  <div
                    className="
                      absolute
