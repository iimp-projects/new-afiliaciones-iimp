...
export function ForgotPasswordForm() {
    const { email, setEmail, isLoading, state, handleSubmit } = useForgotPassword();
    ...
    if (state.success) {
        return (
            <div className="bg-[#f0faeb] border border-[#a2e584] rounded-2xl p-6 text-center animate-fade-in">
                ...
                <h3 className="text-lg font-bold text-[#1f5a04] mb-2">¡Solicitud enviada!</h3>
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="pt-2">
                <button type="submit" disabled={isLoading} className="w-full py-4 rounded-xl text-white font-bold text-base tracking-wide transition-all duration-300 flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed bg-gradient-to-r from-primary via-[#a3722a] to-primary-container shadow-[0_8px_20px_-6px_rgba(127,86,30,0.5)] hover:shadow-[0_12px_25px_-6px_rgba(127,86,30,0.6)] hover:-translate-y-0.5">
