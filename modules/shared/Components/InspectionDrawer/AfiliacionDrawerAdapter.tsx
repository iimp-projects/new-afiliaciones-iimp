...
import { mockDrawerData, mockSmartCaseCards } from "../../../afiliaciones/expedientes/Mocks/ExpedientesMockData";
import { useKeyboardDrawerNavigation } from "../../../afiliaciones/expedientes/Hooks/useKeyboardDrawerNavigation";
import { useTheaterMode } from "@/modules/shared/Components/InspectionDrawer/TheaterModeContext";

// Dummy content para las Tabs
...
const PagosTabContent = () => {
  // Simulando que esta tab expande automáticamente el drawer (Theater Mode)
  const { enableTheaterMode } = useTheaterMode();
  
  // eslint-disable-next-line react-hooks/exhaustive-deps
  // setTimeout(() => enableTheaterMode(), 100); 
  ...
  return (
    <div className="h-full flex flex-col items-center justify-center border-2 border-dashed border-slate-300 rounded-2xl bg-white animate-in fade-in">
      ...
      <p className="text-xs text-slate-400 mt-2">Aquí el Drawer estaría expandido al 85%</p>
    </div>
  );
};
...
export function AfiliacionDrawerAdapter() {
  ...
  // En producción, pasaríamos la lista real de expedientes cargados para que las flechas funcionen
  // Haremos un mock mapeando nuestras SmartCaseCards como "ExpedienteDTO"
  ...
  // Si no hay caso seleccionado, retornamos null (invisible)
  if (!activeCaseCode) return null;

  // En producción, aquí haríamos un fetch a la API con useQuery o swr pasando `activeCaseCode`
  // Por ahora, inyectamos la data mockeada del Quirófano que hicimos en la fase anterior
  const dataToInject = {
    ...mockDrawerData,
    header: mockSmartCaseCards.find(c => c.trackingCode === activeCaseCode) || mockSmartCaseCards[0]
  };

  // Función orquestadora para renderizar el contenido según la pestaña seleccionada
