        ...
        if (v.history && Array.isArray(v.history)) {
          v.history.forEach((vh: any) => {
            timeline.push({
              ...
              user: vh.user?.person
                ? `${vh.user.person.firstName} ${vh.user.person.paternalLastName}`
                : "Revisor del Área",
  ...
  return (
    <>
      ...
      {isAvatarZoomed &&
        createPortal(
          <div
            className="fixed inset-0 z-[9999999] bg-black/95 flex items-center justify-center backdrop-blur-md cursor-zoom-out animate-in fade-in duration-200"
            ...
            <div className="relative flex flex-col items-center justify-center w-full h-full p-4">
              ...
              {header.identity.avatarUrl ? (
                ...
                <div className="scale-[3] sm:scale-[5] pointer-events-none animate-in zoom-in-95 duration-300">
                  <FallbackAvatar
                    identifier={header.trackingCode}
                    initials={header.identity.fallbackInitials}
                    size={84}
                  />
                </div>
              )}
      ...
      <div
        className={`fixed top-0 right-0 h-full bg-white shadow-2xl transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] z-[99999] border-l border-slate-200 overflow-hidden ${isTheaterMode ? "w-full w-[100vw]" : "w-full sm:w-[500px] md:w-[700px] lg:w-[850px]"}`}
      >
        <div className="absolute top-4 right-4 flex items-center gap-2 z-[100]">
          {notificationTab && (
            <button
              onClick={() => setActiveTab(notificationTab.id)}
              title="Ver observaciones pendientes"
              className="relative p-2 text-amber-500 hover:text-amber-600 bg-amber-50 hover:bg-amber-100 rounded-full transition-all shadow-sm border border-amber-200 focus:outline-none"
            >
          ...
          <button
            ...
            title={
              isTheaterMode ? "Reducir panel" : "Expandir a pantalla completa"
            }
            className="p-2 text-slate-500 hover:text-[#C5A059] bg-white border border-slate-200 hover:border-[#C5A059] hover:bg-[#C5A059]/10 rounded-full transition-all shadow-sm focus:outline-none"
          >
            {isTheaterMode ? (
          ...
          <button
            onClick={onClose}
            title="Cerrar"
            className="p-2 text-slate-500 hover:text-red-600 bg-white border border-slate-200 hover:border-red-200 hover:bg-red-50 rounded-full transition-all shadow-sm focus:outline-none"
          >
        ...
        <div
          className={`flex h-full w-full ${isTheaterMode ? "flex-col md:flex-row" : "flex-col"}`}
        >
          <header
            className={`relative bg-white shrink-0 transition-all duration-500 ${isTheaterMode ? "w-full md:w-[320px] lg:w-[400px] h-auto md:h-full overflow-y-auto border-b md:border-b-0 md:border-r border-slate-200 p-6 sm:p-8 pt-20 flex flex-col items-center text-center md:items-start md:text-left gap-5 bg-[#fdfdfd]" : "w-full border-b border-slate-50 px-5 sm:px-8 pt-16 pb-6 flex flex-row items-start gap-5"}`}
          >
            <div
              onClick={() => setIsAvatarZoomed(true)}
              className={`rounded-full overflow-hidden border border-slate-200 shrink-0 shadow-sm relative bg-slate-50 cursor-zoom-in group ${isTheaterMode ? "w-24 h-24 sm:w-32 sm:h-32 mb-2" : "w-16 h-16 sm:w-[84px] sm:h-[84px] mt-1"}`}
            >
            ...
            >
              ...
              <div
                className={`flex ${isTheaterMode ? "flex-col items-center md:items-start" : "flex-wrap items-center"} gap-3 mb-1 w-full`}
              >
                <h2
                  className={`font-black text-slate-800 leading-tight capitalize ${isTheaterMode ? "text-2xl sm:text-3xl" : "text-[18px] sm:text-[22px] truncate"}`}
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-50 text-blue-600 hover:bg-blue-500 hover:text-white rounded-full transition-all shadow-sm font-bold text-[11px] uppercase tracking-wider focus:outline-none group w-max"
                >
                  <History
                    size={14}
                    strokeWidth={2.5}
              ...
              {/* ===================================================================== */}
              <div
                className={`flex flex-wrap items-center gap-4 mt-1 ${isTheaterMode ? "justify-center md:justify-start" : ""}`}
              >
                {/* DNI + Botón Copiar */}
                <div className="flex items-center gap-1.5 border-r border-slate-200 pr-4">
                  <p className="text-[13px] font-black text-slate-700 font-mono truncate">
                  ...
                  <div className="flex items-center gap-1.5 text-slate-500">
                    ...
                    <span className="font-bold">
                      ...
                      <span className="text-slate-700 uppercase">
                        {header.metadata.reviewerArea || "Asignada"}
                      </span>
                    </span>
                  </div>
                    ...
                    {header.metadata.isAlreadyEvaluatedByMe ? (
                      <>
                        ...
                        <span className="font-bold">
                          Evaluado por:{" "}
                          <span className="text-slate-700 uppercase">
                            {header.metadata.assignedTo?.name ||
                              "Administrador"}
                          </span>
                        </span>
                      </>
                      ...
                      <>
                        ...
                        <span className="font-bold">
                          Por evaluar (
                          <span className="text-slate-700 uppercase">
                            {header.metadata.assignedTo?.name?.split(" ")[0] ||
                              "Administrador"}
                          </span>
                          )
                        </span>
              ...
              {evaluationActions && (
                <div
                  className={`mt-5 pt-5 border-t border-slate-100 flex ${isTheaterMode ? "flex-col w-full" : "flex-wrap items-center"} gap-3`}
                >
                  {evaluationActions}
                </div>
              )}
            ...
            >
              {availableTabs.map((tab) => (
                <button
                  ...
                  className={`relative flex-1 min-w-[130px] py-3.5 px-2 text-[13px] sm:text-[14px] font-bold transition-all whitespace-nowrap focus:outline-none flex items-center justify-center rounded-t-xl ${
                    activeTab === tab.id
                      ? "text-[#C5A059] bg-slate-50"
                      : "text-slate-500 hover:text-[#C5A059] hover:bg-slate-50/60"
                  }`}
                >
                  <span>{tab.label}</span>

                  {tab.hasNotification && (
                    <span className="ml-1.5 mb-2 w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse shadow-sm" />
                  )}

                  {activeTab === tab.id && (
                    <div className="absolute -bottom-[2px] left-0 w-full h-[3px] bg-[#C5A059] rounded-t-md z-10" />
                  )}
                </button>
              ))}
            </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs font-semibold text-slate-500">
                    Historial detallado por áreas y sistema
                  </p>
                </div>
              </div>
              <button
                      ...
                      >
                        ...
                        <div className="bg-white border border-slate-200 hover:border-blue-300 rounded-2xl p-4 shadow-sm hover:shadow-md transition-all duration-300 ml-4 group-hover:bg-blue-50/30">
                          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3">
                            <span
                              className={`px-2.5 py-1 rounded-md text-[10px] font-black uppercase tracking-widest w-max ${item.isSystem ? "bg-slate-100 text-slate-600" : "bg-blue-100 text-blue-700"}`}
                            >
                              {item.area}
                            </span>
                            <div className="flex items-center gap-1.5 text-slate-400 text-[11px] font-bold bg-slate-50 px-2 py-1 rounded-lg border border-slate-100 w-max">
